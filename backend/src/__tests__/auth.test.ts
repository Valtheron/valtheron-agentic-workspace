...
describe('Auth Endpoints', () => {
  ...
  describe('POST /api/auth/login', () => {
    ...
    it('rejects invalid password', async () => {
      const res = await request(app).post('/api/auth/login').send({ username: testUser.username, password: 'wrong' });
  ...
  describe('POST /api/auth/refresh', () => {
    it('issues a new token when valid token provided', async () => {
      const loginRes = await request(app).post('/api/auth/login').send(testUser);

      const res = await request(app).post('/api/auth/refresh').set('Authorization', `Bearer ${loginRes.body.token}`);
