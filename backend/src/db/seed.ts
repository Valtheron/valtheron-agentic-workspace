...
const CATEGORIES = ['trading', 'security', 'development', 'qa', 'documentation', 'deployment', 'analyst', 'support', 'integration', 'monitoring'] as const;
...
const ROLES: Record<string, string[]> = {
  ...
  development: ['Code Generation', 'Architecture Design', 'API Development', 'Frontend Development', 'Backend Development'],
  qa: ['Test Automation', 'Quality Assurance', 'Performance Testing', 'Security Testing', 'Regression Testing'],
  documentation: ['Technical Writing', 'API Documentation', 'User Guide Creation', 'Knowledge Management', 'Changelog Generation'],
  deployment: ['Container Orchestration', 'CI/CD Pipeline', 'Infrastructure Management', 'Release Management', 'Configuration Management'],
  ...
  integration: ['API Integration', 'Data Synchronization', 'Webhook Management', 'ETL Processing', 'Service Connection'],
...
export function seedDatabase() {
  ...
  db.prepare('INSERT INTO users (id, username, passwordHash, role) VALUES (?, ?, ?, ?)').run(
    adminId, 'admin', hashPassword('valtheron2024'), 'admin'
  );
  db.prepare('INSERT INTO users (id, username, passwordHash, role) VALUES (?, ?, ?, ?)').run(
  ...
  const seedAgents = db.transaction(() => {
    for (const category of CATEGORIES) {
      ...
      for (let i = 0; i < names.length; i++) {
        ...
        insertAgent.run(
          id, name, role, category, status,
          +successRate.toFixed(1), tasksCompleted, failedTasks, +avgTaskDuration.toFixed(0),
          new Date(Date.now() - Math.random() * 86400000).toISOString(),
          `Du bist ${name}, ein spezialisierter ${role}-Agent der Kategorie ${category}. Arbeite präzise und effizient.`,
          personality, parameters, '[]', '[]',
          'anthropic', 'claude-sonnet-4-5-20250929', riskProfile
        );
  ...
  const KANBAN_COLS = ['backlog', 'todo', 'in_progress', 'review', 'done'];
  const TASK_TITLES: Record<string, string[]> = {
    trading: ['Implement moving average crossover', 'Fix order execution latency', 'Add risk limit alerts', 'Backtest momentum strategy'],
    security: ['Run vulnerability scan', 'Update firewall rules', 'Audit access permissions', 'Implement rate limiting'],
    development: ['Build REST API endpoints', 'Refactor auth module', 'Add WebSocket support', 'Optimize database queries'],
    qa: ['Write E2E tests for checkout', 'Load test API endpoints', 'Fix flaky unit tests', 'Add accessibility tests'],
    documentation: ['Update API documentation', 'Write deployment guide', 'Create onboarding tutorial', 'Generate changelog'],
    deployment: ['Set up Kubernetes cluster', 'Configure CI/CD pipeline', 'Implement blue-green deploy', 'Add health checks'],
    analyst: ['Create revenue dashboard', 'Analyze user retention', 'Build churn prediction model', 'Generate monthly report'],
    support: ['Set up ticket routing', 'Create FAQ knowledge base', 'Implement chatbot flow', 'Design escalation rules'],
    integration: ['Connect Slack webhook', 'Build ETL pipeline', 'Implement SSO login', 'Add payment gateway'],
    monitoring: ['Set up Prometheus alerts', 'Configure log aggregation', 'Build status dashboard', 'Add latency tracking'],
  };
    ...
    for (const category of CATEGORIES) {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        insertTask.run(
          ...
          status, PRIORITIES[Math.floor(Math.random() * PRIORITIES.length)],
          agent.id, category, kanban,
  ...
  const sampleWorkflows = [
    {
      ...
      status: 'draft',
      steps: [
        { id: uuid(), name: 'Code Review', description: 'Automatische Code-Analyse', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 120, retries: 0 },
        { id: uuid(), name: 'Unit Tests', description: 'Alle Unit-Tests ausführen', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 300, retries: 0 },
        { id: uuid(), name: 'Build', description: 'Production Build erstellen', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 180, retries: 0 },
        { id: uuid(), name: 'Security Scan', description: 'Vulnerability Scan durchführen', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 240, retries: 0 },
        { id: uuid(), name: 'Deploy Staging', description: 'Auf Staging deployen', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 60, retries: 0 },
        { id: uuid(), name: 'E2E Tests', description: 'End-to-End Tests auf Staging', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 600, retries: 0 },
        { id: uuid(), name: 'Deploy Production', description: 'Blue-Green Deployment in Production', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 120, retries: 0 },
      ],
    ...
    },
    {
      ...
      status: 'draft',
      steps: [
        { id: uuid(), name: 'Threat Assessment', description: 'Bedrohungsanalyse durchführen', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 300, retries: 0 },
        { id: uuid(), name: 'Vulnerability Scan', description: 'Automatischer Vulnerability Scan', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 600, retries: 0 },
        { id: uuid(), name: 'Penetration Test', description: 'Simulierter Angriff', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 1200, retries: 0 },
        { id: uuid(), name: 'Compliance Check', description: 'GDPR & SOC2 Compliance prüfen', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 300, retries: 0 },
        { id: uuid(), name: 'Report Generation', description: 'Sicherheitsbericht erstellen', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 180, retries: 0 },
      ],
    ...
    },
    {
      ...
      status: 'draft',
      steps: [
        { id: uuid(), name: 'Data Collection', description: 'Daten aus allen Quellen sammeln', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 180, retries: 0 },
        { id: uuid(), name: 'Data Cleaning', description: 'Datenbereinigung und Normalisierung', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 300, retries: 0 },
        { id: uuid(), name: 'Analysis', description: 'Statistische Analyse durchführen', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 600, retries: 0 },
        { id: uuid(), name: 'Visualization', description: 'Dashboards und Charts erstellen', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 240, retries: 0 },
        { id: uuid(), name: 'Report', description: 'Ergebnisbericht generieren', assignedAgentId: null, status: 'pending', dependsOn: [], output: null, progress: 0, estimatedDuration: 120, retries: 0 },
      ],
  ...
  const secEvents = [
    ...
    { type: 'injection', severity: 'critical', message: 'Prompt injection attempt detected in Agent input', resolved: 1 },
    ...
    { type: 'policy', severity: 'medium', message: 'Data retention policy violation in analytics pipeline', resolved: 0 },
  ];

  const seedEvents = db.transaction(() => {
    for (const ev of secEvents) {
      insertSecEvent.run(uuid(), ev.type, ev.severity, ev.message, null, new Date(Date.now() - Math.random() * 86400000 * 7).toISOString(), ev.resolved);
    }
  });
  seedEvents();
