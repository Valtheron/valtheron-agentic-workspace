...
function generateSLAs(): SLAMetric[] {
  return [
    { id: 's1', name: 'Response Time', metric: 'response_time', threshold: 200, unit: 'ms', current: 168, status: 'met', period: 'daily', history: Array.from({ length: 24 }, (_, i) => ({ timestamp: `${i}:00`, value: 140 + Math.random() * 80 })) },
    { id: 's2', name: 'Success Rate', metric: 'success_rate', threshold: 95, unit: '%', current: 96.2, status: 'met', period: 'daily', history: Array.from({ length: 24 }, (_, i) => ({ timestamp: `${i}:00`, value: 93 + Math.random() * 5 })) },
    { id: 's3', name: 'System Uptime', metric: 'uptime', threshold: 99.5, unit: '%', current: 99.8, status: 'met', period: 'monthly', history: Array.from({ length: 30 }, (_, i) => ({ timestamp: `Tag ${i + 1}`, value: 99 + Math.random() })) },
    { id: 's4', name: 'Throughput', metric: 'throughput', threshold: 50, unit: 'tasks/h', current: 47, status: 'warning', period: 'hourly', history: Array.from({ length: 24 }, (_, i) => ({ timestamp: `${i}:00`, value: 35 + Math.random() * 25 })) },
    { id: 's5', name: 'Error Rate', metric: 'error_rate', threshold: 5, unit: '%', current: 6.1, status: 'breached', period: 'daily', history: Array.from({ length: 24 }, (_, i) => ({ timestamp: `${i}:00`, value: 3 + Math.random() * 5 })) },
  ];
}

// SVG Mini Chart
function LineChart({ data, width = 300, height = 120, color = 'var(--accent-cyan)', showDots = false }: { data: number[]; width?: number; height?: number; color?: string; showDots?: boolean }) {
...
function BarChart({ data, labels, width = 300, height = 140, color = 'var(--accent-cyan)' }: { data: number[]; labels?: string[]; width?: number; height?: number; color?: string }) {
      ...
      {data.map((v, i) => {
        ...
        return (
          <g key={i}>
            ...
            {labels && labels[i] && (
              <text x={x + barW / 2} y={height + 14} textAnchor="middle" fill="var(--text-muted)" fontSize="9">{labels[i].slice(5)}</text>
            )}
          </g>
        );
      })}
...
export default function AnalyticsView({ analytics, agents, tasks }: AnalyticsProps) {
  ...
  const slaColors: Record<string, string> = { met: 'var(--accent-green)', warning: 'var(--accent-orange)', breached: 'var(--accent-red)' };

  return (
    <div>
      <div className="tabs">
        {([['trends', 'Performance Trends'], ['throughput', 'Durchsatz'], ['errors', 'Fehlerrate'], ['capacity', 'Capacity Planning'], ['sla', 'SLA Monitoring'], ['success', 'Erfolgsrate']] as const).map(([k, l]) => (
        ...
        <div>
          <div className="kpi-grid">
            <div className="kpi-card">
              ...
              <div className="kpi-value cyan">{Math.round(trends.reduce((s, t) => s + t.throughput, 0) / trends.length)}</div>
              <div className="kpi-sub">Tasks/Tag</div>
            </div>
            <div className="kpi-card">
              <div className="kpi-label">Avg Response Time</div>
              <div className="kpi-value">{Math.round(trends.reduce((s, t) => s + t.avgResponseTime, 0) / trends.length)}ms</div>
            </div>
            <div className="kpi-card">
              <div className="kpi-label">Avg Erfolgsrate</div>
              <div className="kpi-value green">{(trends.reduce((s, t) => s + t.successRate, 0) / trends.length).toFixed(1)}%</div>
            </div>
            <div className="kpi-card">
              <div className="kpi-label">Avg Fehlerrate</div>
              <div className="kpi-value red">{(trends.reduce((s, t) => s + t.errorRate, 0) / trends.length).toFixed(1)}%</div>
            </div>
          </div>
          <div className="grid-2">
            ...
            <div className="card">
              ...
              <LineChart data={trends.map(t => t.avgResponseTime)} width={500} height={150} color="var(--accent-orange)" />
            </div>
            <div className="card">
      ...
      {tab === 'throughput' && (
        <div>
          <div className="card mb-16">
            ...
            <BarChart data={trends.map(t => t.throughput)} labels={trends.map(t => t.date)} width={800} height={200} />
          </div>
          <div className="grid-2">
            <div className="card">
              ...
              {Object.entries(catCounts).map(([cat, c]) => (
                <div key={cat} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  ...
                  <div style={{ flex: 1, height: 8, background: 'var(--bg-hover)', borderRadius: 4, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${(c.total / agents.length) * 100}%`, background: 'var(--accent-cyan)', borderRadius: 4 }} />
                  </div>
            ...
            <div className="card">
              ...
              {[...agents].sort((a, b) => b.tasksCompleted - a.tasksCompleted).slice(0, 10).map((a, i) => (
      ...
      {tab === 'errors' && (
        <div>
          <div className="card mb-16">
            ...
            <LineChart data={trends.map(t => t.errorRate)} width={800} height={200} color="var(--accent-red)" showDots />
          ...
          <div className="grid-2">
            <div className="card">
              ...
              {Object.entries(catCounts).map(([cat]) => {
                ...
                const avgFail = catAgents.length ? catAgents.reduce((s, a) => s + a.failedTasks, 0) / catAgents.length : 0;
                return (
                  <div key={cat} className="config-row">
                    <span className="config-label">{cat}</span>
                    <span style={{ fontWeight: 600, color: avgFail > 10 ? 'var(--accent-red)' : 'var(--text-secondary)' }}>{avgFail.toFixed(1)} avg</span>
                  </div>
                );
              })}
            </div>
            <div className="card">
              ...
              {[...agents].sort((a, b) => (b.failedTasks / (b.tasksCompleted + b.failedTasks || 1)) - (a.failedTasks / (a.tasksCompleted + a.failedTasks || 1))).slice(0, 10).map(a => (
                <div key={a.id} className="config-row">
        ...
        <div>
          <div className="kpi-grid">
            ...
            <div className="kpi-card">
              ...
              <div className="kpi-value orange">{((agents.filter(a => a.status === 'working' || a.status === 'active').length / agents.length) * 100).toFixed(0)}%</div>
            </div>
            <div className="kpi-card">
            ...
            <div className="kpi-card">
              ...
              <div className="kpi-value red">{agents.filter(a => a.status === 'blocked' || a.status === 'error').length}</div>
            </div>
          </div>
          <div className="card mb-16">
              ...
              {Object.entries(catCounts).map(([cat, c]) => {
                ...
                return (
                  <div key={cat} className="card" style={{ padding: 12 }}>
                    ...
                    <div style={{ fontSize: 20, fontWeight: 700, color: util > 80 ? 'var(--accent-red)' : util > 50 ? 'var(--accent-orange)' : 'var(--accent-green)' }}>
                      {util.toFixed(0)}%
                    </div>
                    <div style={{ height: 6, background: 'var(--bg-hover)', borderRadius: 3, marginTop: 6, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${util}%`, background: util > 80 ? 'var(--accent-red)' : util > 50 ? 'var(--accent-orange)' : 'var(--accent-green)', borderRadius: 3 }} />
                    </div>
            ...
            {slas.map(sla => (
              <div key={sla.id} className="card" style={{ borderLeft: `3px solid ${slaColors[sla.status]}` }}>
                <div className="flex-between mb-8">
                  ...
                  <span className={`badge ${sla.status === 'met' ? 'valid' : sla.status === 'warning' ? 'high' : 'critical'}`}>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>
                  Threshold: {sla.threshold}{sla.unit} | {sla.period}
                </div>
                <LineChart data={sla.history.map(h => h.value)} width={220} height={60} color={slaColors[sla.status]} />
              </div>
            ))}
          </div>
        </div>
      ...
      {tab === 'success' && (
        <div>
          <div className="card mb-16">
            ...
            <LineChart data={trends.map(t => t.successRate)} width={800} height={200} color="var(--accent-green)" showDots />
          </div>
          <div className="card">
            ...
            {successByAgent.map((a, i) => (
              <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <div style={{ width: 200, height: 8, background: 'var(--bg-hover)', borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${a.successRate}%`, background: a.successRate >= 97 ? 'var(--accent-green)' : a.successRate >= 90 ? 'var(--accent-cyan)' : 'var(--accent-orange)', borderRadius: 4 }} />
                </div>
                <span style={{ fontSize: 11, fontWeight: 600, width: 40, textAlign: 'right', color: a.successRate >= 97 ? 'var(--accent-green)' : 'var(--text-secondary)' }}>
                  {a.successRate}%
                </span>
              </div>
            ))}
