      ...
      <div style={{ overflow: 'auto' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          {['all', 'valid', 'expiring', 'expired', 'suspended', 'revoked'].map((f) => (
            ...
            >
              {f === 'all' ? 'Alle' : f} (
              {f === 'all' ? certifications.length : certifications.filter((c) => c.status === f).length})
            </button>
          ))}
        </div>
        ...
        {certifications.length === 0 ? (
          <div className="card" style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
            <div style={{ fontSize: 18, marginBottom: 8 }}>Noch keine Zertifizierungen</div>
            <div style={{ fontSize: 12 }}>
              Zertifizierungen erscheinen hier, sobald Agenten ihren Test-Zyklus durchlaufen haben. Aktuell ist kein
              Zertifizierungs-Backend angebunden.
            </div>
          </div>
        ) : (
            ...
            {filtered.map((cert) => (
              ...
              >
                ...
                <div style={{ textAlign: 'center' }}>
                  <span className={`badge ${cert.level}`}>{cert.level}</span>
                </div>
        ...
        <div className="card" style={{ overflow: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16, fontSize: 12 }}>
            ...
            <div>
              ...
              <span style={{ fontWeight: 700, color: 'var(--accent-orange)' }}>{selected.score}</span>
            </div>
            <div>
              <span style={{ color: 'var(--text-muted)' }}>Ausgestellt:</span>{' '}
              {new Date(selected.issuedAt).toLocaleDateString('de-DE')}
            </div>
          </div>
