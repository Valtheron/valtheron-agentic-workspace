...
import { collaborationAPI, getLLMHeaders } from '../services/api';
...
export default function CollaborationView({ agents }: CollabProps) {
  ...
  const handleStatusChange = async (id: string, status: string) => {
    try {
      ...
      setSessions((prev) =>
        prev.map((s) => (s.id === id ? { ...s, status: status as CollaborationSession['status'] } : s)),
      );
    } catch {
      /* ignore */
    }
  ...
  const handleRun = async () => {
    ...
    if (!headers) {
      setRunError(
        'Kein aktiver LLM-Provider mit API-Key. Hinterlege einen Key unter "LLM Provider" — es wird nichts simuliert.',
      );
      return;
    }
    setRunError(null);
    setRunning(true);
    try {
      const res = await collaborationAPI.run(selectedId, headers);
      await loadMessages(selectedId);
      setSessions((prev) =>
        prev.map((s) =>
          s.id === selectedId
            ? { ...s, status: 'completed' as CollaborationSession['status'], synthesis: res.synthesis }
            : s,
        ),
      );
    } catch (e) {
      setRunError(e instanceof Error ? e.message : 'Orchestrierung fehlgeschlagen.');
    } finally {
      setRunning(false);
    }
  };
        ...
        <div className="collab-session-list">
          ...
          {!loading && sessions.length === 0 && (
            <div style={{ padding: 24, textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}>
              ...
              <br />
              Erstelle eine neue Collaboration!
            </div>
          )}
            ...
            >
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 4 }}>
                  ...
                  <span className={`badge ${session.status}`} style={{ flexShrink: 0 }}>
                    {session.status}
                  </span>
                </div>
        ...
        {showCreate ? (
          <div className="collab-create-panel">
            ...
            <div className="collab-form">
              ...
              <input
                ...
                placeholder="z.B. Security Audit Sprint"
              />
              ...
              <textarea
                ...
                placeholder="Beschreibe die Aufgabe der Collaboration..."
                rows={3}
              />
              ...
              <div style={{ display: 'flex', gap: 16 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <select
                    ...
                    value={strategy}
                    onChange={(e) => setStrategy(e.target.value as CollaborationSession['delegationStrategy'])}
                  >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <select
                    className="collab-select"
                    value={conflict}
                    onChange={(e) => setConflict(e.target.value as CollaborationSession['conflictResolution'])}
                  >
                ...
                {filteredAgents.slice(0, 60).map((a) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        setSelectedAgents((prev) => (isSel ? prev.filter((x) => x !== a.id) : [...prev, a.id]))
                      }
                    >
                      {a.name}
              ...
              <button
                ...
                disabled={!newName.trim() || selectedAgents.length < 2}
          ...
          <div className="chat-empty">
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Wähle eine Session oder erstelle eine neue Collaboration
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="collab-detail-header">
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>
                  {selectedSession.delegationStrategy} · {selectedSession.conflictResolution} · Konsens:{' '}
                  {selectedSession.consensusThreshold}%
                </div>
              </div>
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                ...
                {selectedSession.status === 'active' && (
                  <button
                    ...
                    onClick={handleRun}
                    disabled={running || !selectedSession.coordinatorPrompt}
                    title={
                      selectedSession.coordinatorPrompt
                        ? 'Agenten per LLM auf den Koordinator-Prompt antworten lassen'
                        : 'Koordinator-Prompt erforderlich'
                    }
                  >
            ...
            {running && (
              <div style={{ margin: '8px 0', padding: '8px 12px', fontSize: 12, color: 'var(--text-muted)' }}>
                Agenten arbeiten an der Aufgabe… (echte LLM-Aufrufe, das kann einige Sekunden dauern)
              </div>
            )}
                ...
                <div key={msg.id} className={`collab-msg${msg.messageType !== 'message' ? ' special' : ''}`}>
                  ...
                  <div className="collab-msg-body">
                    <div className="collab-msg-header">
                      <span style={{ fontWeight: 600, fontSize: 12, color: 'var(--accent-cyan)' }}>
                        {getAgentName(msg.senderId)}
                      </span>
                      <span style={{ fontSize: 10, color: 'var(--text-muted)', marginLeft: 6 }}>
                        {getAgentCategory(msg.senderId)}
                      </span>
            ...
            {selectedSession.status === 'active' && (
              <div className="collab-input-area">
                <select
                  className="collab-sender-select"
                  value={msgSender}
