...
const defaultRisk: AgentRiskProfile = {
  riskLevel: 'medium', maxConcurrentTasks: 5, maxTokenBudget: 100000,
  autoSuspendOnFailure: false, failureThreshold: 3, cooldownPeriod: 60, requiresApproval: false,
};

const riskColors: Record<string, string> = {
  low: 'var(--accent-green)', medium: 'var(--accent-orange)', high: 'var(--accent-red)', critical: '#ff1744',
};

function genId() { return `kse_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`; }

export default function KillSwitchView({ killSwitch, agents, onToggleKillSwitch, onUpdateKillSwitch, onUpdateAgents }: KillSwitchViewProps) {
  ...
  const addEvent = (action: KillSwitchEvent['action'], reason: string, affected: string[]) => {
    const evt: KillSwitchEvent = { id: genId(), action, triggeredBy: 'admin', reason, affectedAgents: affected, timestamp: new Date().toISOString() };
    onUpdateKillSwitch({ ...killSwitch, history: [evt, ...history] });
  };

  const handleConfirmToggle = () => {
    const action = killSwitch.armed ? 'disarmed' : 'armed';
    addEvent(action, confirmReason || (action === 'armed' ? 'Manual activation' : 'Manual deactivation'), killSwitch.affectedAgents);
    onToggleKillSwitch();
  ...
  return (
    <div>
      <div className="tabs">
        {([['panel', 'Kill-Switch'], ['history', 'History Log'], ['risk', 'Risiko-Parameter'], ['batch', 'Batch Operations']] as const).map(([k, l]) => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, padding: '40px 0' }}>
          ...
          <button
            ...
            style={{
              ...
              color: 'white', fontWeight: 800, fontSize: 18, textTransform: 'uppercase', letterSpacing: 2,
          ...
          <div style={{ textAlign: 'center', maxWidth: 400 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
              {agents.filter(a => a.status === 'active' || a.status === 'working').length} aktive Agenten &middot; {killSwitch.autoTriggerRules.filter(r => r.enabled).length} Auto-Trigger aktiv
            </div>
          </div>
            ...
            {killSwitch.autoTriggerRules.map(rule => (
              <div key={rule.id} className="config-row">
                ...
                <button
                  ...
                  onClick={() => {
                    const rules = killSwitch.autoTriggerRules.map(r => r.id === rule.id ? { ...r, enabled: !r.enabled } : r);
          ...
          {confirmOpen && (
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div className="card" style={{ width: 420, textAlign: 'center' }}>
                <div style={{ fontSize: 16, fontWeight: 700, color: killSwitch.armed ? 'var(--accent-green)' : 'var(--accent-red)', marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
                  <button className="btn btn-ghost" onClick={() => setConfirmOpen(false)}>Abbrechen</button>
                  <button className={`btn ${killSwitch.armed ? 'btn-primary' : 'btn-danger'}`} onClick={handleConfirmToggle}>
      ...
      {tab === 'history' && (
        <div className="card">
          ...
          {history.length === 0 ? (
            <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}>Keine Events vorhanden</div>
          ) : (
            <div className="table-wrap">
              <table>
                <thead>
                  <tr><th>Zeit</th><th>Aktion</th><th>Grund</th><th>Von</th><th>Betroffene Agenten</th></tr>
                </thead>
                <tbody>
                  {history.slice(0, 50).map(evt => (
                    <tr key={evt.id}>
                      <td>{new Date(evt.timestamp).toLocaleString('de-DE')}</td>
                      <td><span className={`badge ${evt.action === 'armed' || evt.action === 'triggered' ? 'critical' : evt.action === 'disarmed' ? 'valid' : 'info'}`}>{evt.action}</span></td>
                      <td>{evt.reason || '-'}</td>
          ...
          <div className="card" style={{ maxHeight: 'calc(100vh - 200px)', overflowY: 'auto' }}>
            ...
            {agents.map(a => (
              <div
                ...
                style={{ cursor: 'pointer', padding: '8px 4px', borderRadius: 4, background: riskAgent === a.id ? 'var(--bg-hover)' : 'transparent' }}
              ...
              >
                ...
                <span style={{ fontSize: 11, color: riskColors[(a.riskProfile ?? defaultRisk).riskLevel], fontWeight: 600 }}>
              ...
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div>
                    ...
                    <select value={currentRisk.riskLevel} onChange={e => updateRiskProfile(riskAgentData.id, { ...currentRisk, riskLevel: e.target.value as AgentRiskProfile['riskLevel'] })} style={{ width: '100%' }}>
                      {(['low', 'medium', 'high', 'critical'] as const).map(l => <option key={l} value={l}>{l.toUpperCase()}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={{ fontSize: 11, color: 'var(--text-muted)' }}>Max Concurrent Tasks</label>
                    <input type="number" value={currentRisk.maxConcurrentTasks} onChange={e => updateRiskProfile(riskAgentData.id, { ...currentRisk, maxConcurrentTasks: +e.target.value })} style={{ width: '100%' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: 11, color: 'var(--text-muted)' }}>Max Token Budget</label>
                    <input type="number" value={currentRisk.maxTokenBudget} onChange={e => updateRiskProfile(riskAgentData.id, { ...currentRisk, maxTokenBudget: +e.target.value })} style={{ width: '100%' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: 11, color: 'var(--text-muted)' }}>Failure Threshold</label>
                    <input type="number" value={currentRisk.failureThreshold} onChange={e => updateRiskProfile(riskAgentData.id, { ...currentRisk, failureThreshold: +e.target.value })} style={{ width: '100%' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: 11, color: 'var(--text-muted)' }}>Cooldown (Sekunden)</label>
                    <input type="number" value={currentRisk.cooldownPeriod} onChange={e => updateRiskProfile(riskAgentData.id, { ...currentRisk, cooldownPeriod: +e.target.value })} style={{ width: '100%' }} />
                  </div>
                  <div className="config-row">
                    <span className="config-label">Auto-Suspend bei Fehler</span>
                    <button className={`toggle${currentRisk.autoSuspendOnFailure ? ' on' : ''}`} onClick={() => updateRiskProfile(riskAgentData.id, { ...currentRisk, autoSuspendOnFailure: !currentRisk.autoSuspendOnFailure })} />
                  </div>
                  <div className="config-row">
                    <span className="config-label">Genehmigung erforderlich</span>
                    <button className={`toggle${currentRisk.requiresApproval ? ' on' : ''}`} onClick={() => updateRiskProfile(riskAgentData.id, { ...currentRisk, requiresApproval: !currentRisk.requiresApproval })} />
                  </div>
                </div>
              </>
            ) : (
              <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}>Agent auswaehlen um Risiko-Parameter zu bearbeiten</div>
            )}
          </div>
        </div>
      )}
      ...
      {tab === 'batch' && (
        <div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 16, alignItems: 'center', flexWrap: 'wrap' }}>
            <select value={batchFilter} onChange={e => setBatchFilter(e.target.value)} style={{ padding: '6px 10px' }}>
            ...
            <div style={{ flex: 1 }} />
            <button className="btn btn-primary btn-sm" disabled={selectedAgents.size === 0} onClick={() => batchSetStatus('active')}>
              Batch Start ({selectedAgents.size})
            </button>
            <button className="btn btn-danger btn-sm" disabled={selectedAgents.size === 0} onClick={() => batchSetStatus('suspended')}>
              Batch Stop ({selectedAgents.size})
            </button>
            <button className="btn btn-ghost btn-sm" disabled={selectedAgents.size === 0} onClick={() => {
                ...
                <tbody>
                  {filteredAgents.map(a => (
                    <tr key={a.id} onClick={() => toggleSelect(a.id)} style={{ cursor: 'pointer' }}>
                      ...
                      <td><span className="kanban-tag">{a.category}</span></td>
                      <td><span className={`badge ${a.status}`}>{a.status}</span></td>
                      <td>{a.successRate}%</td>
                      <td>
                        <span style={{ color: riskColors[(a.riskProfile ?? defaultRisk).riskLevel], fontWeight: 600, fontSize: 11 }}>
                          {(a.riskProfile ?? defaultRisk).riskLevel.toUpperCase()}
                        </span>
                      </td>
