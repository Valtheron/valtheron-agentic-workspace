  ...
  const updateProviderStatus = (id: LLMProviderType, status: LLMProvider['status']) => {
    onConfigChange({
      ...
      providers: config.providers.map((p) =>
        p.id === id ? { ...p, status, lastChecked: new Date().toISOString() } : p,
      ),
    });
  };
  ...
  const loadOllamaModels = async () => {
    ...
    try {
      ...
      onConfigChange({
        ...
        ollamaModels,
        providers: config.providers.map((p) =>
          p.id === 'ollama' ? { ...p, models: llmModels, status: 'connected' } : p,
        ),
      });
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 12 }}>
          {config.providers.map((provider) => (
            <div
              key={provider.id}
              className="card"
              style={{ borderColor: provider.enabled ? 'var(--border-active)' : 'var(--border-color)' }}
            >
                  ...
                  {editingProvider === provider.id ? (
                    <div style={{ display: 'flex', gap: 4 }}>
                      ...
                      <button
                        ...
                        onClick={() => {
                          setApiKey(provider.id, apiKeyInput);
                ...
                <div style={{ marginBottom: 8 }}>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setProviderUrl('custom', e.target.value);
                    }}
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    ...
                    />
                    <button className="btn btn-primary btn-sm" onClick={addCustomModel}>
                      +
                    </button>
                  </div>
                </div>
        ...
        <div className="grid-2">
          <div className="card">
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <span
                  style={{
                    fontSize: 12,
                    fontWeight: 600,
                    color: ollamaStatus === 'connected' ? 'var(--accent-green)' : 'var(--text-muted)',
                  }}
                >
                  {ollamaStatus === 'checking'
                    ? 'Prüfe...'
            ...
            <div style={{ marginTop: 12, fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.6 }}>
              Stellen Sie sicher, dass Ollama lokal läuft:
              <br />
              <code style={{ background: 'var(--bg-surface)', padding: '2px 6px', borderRadius: 4 }}>ollama serve</code>
              <br />
                ...
                {config.ollamaModels.map((model) => (
                  ...
                  >
                    <div>
                      ...
                      <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>
                        {model.details.family} &middot; {model.details.parameterSize} &middot;{' '}
                ...
                <div style={{ marginTop: 12, padding: 10, background: 'var(--bg-surface)', borderRadius: 6 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                    ...
                    {model.contextWindow >= 1000000
                      ? `${(model.contextWindow / 1000000).toFixed(0)}M`
                      : `${(model.contextWindow / 1000).toFixed(0)}k`}{' '}
                    tokens &middot; Max Output: {(model.maxOutput / 1000).toFixed(0)}k
                  </div>
                  <div style={{ display: 'flex', gap: 4, marginTop: 6, flexWrap: 'wrap' }}>
                    {model.capabilities.map((cap) => (
                      <span key={cap} className="badge active">
                        {cap}
                      </span>
                    ))}
                  </div>
                    ...
                    {provider.models.map((model) => (
                      <div
                        ...
                        style={{
                          ...
                          borderLeft:
                            config.defaultModel === model.id ? '2px solid var(--accent-cyan)' : '2px solid transparent',
                        }}
                        onClick={() =>
                          onConfigChange({ ...config, defaultProvider: provider.id, defaultModel: model.id })
                        }
                      >
                        <span
                          style={{
                            color: config.defaultModel === model.id ? 'var(--accent-cyan)' : 'var(--text-primary)',
                          }}
                        >
                          {model.name}
                        </span>
          ...
          <div className="card">
            ...
            {(
              ...
              <div key={param.key} style={{ marginBottom: 12 }}>
                ...
                />
                <div
                  style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--text-muted)' }}
                >
                  <span>{param.min}</span>
                  <span>{param.max}</span>
                </div>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  onConfigChange({
                    ...config,
                    globalParameters: { ...config.globalParameters, maxRetries: parseInt(e.target.value) },
                  })
                }
                style={{ width: '100%', accentColor: 'var(--accent-cyan)' }}
              />
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  onConfigChange({
                    ...config,
                    globalParameters: { ...config.globalParameters, timeoutMs: parseInt(e.target.value) },
                  })
                }
                style={{ width: '100%', accentColor: 'var(--accent-cyan)' }}
              />
