...
export default function ProjectsView({
  ...
  const handleImportUrl = async () => {
    ...
    try {
      ...
      setImportStatus(
        `Fehler: ${e instanceof Error ? e.message : 'Unbekannter Fehler'}. URL-Analyse aus URL-Struktur wird verwendet.`,
      );
            ...
            <div className="card" style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  {selected.sourceUrl && (
                    <div style={{ fontSize: 11, color: 'var(--accent-cyan)', marginTop: 2 }}>{selected.sourceUrl}</div>
                  )}
              ...
              <div className="grid-2">
                <div className="card">
                  <div className="card-title mb-8">
                    Erkannte Features ({selected.analyzedStructure.features.length})
                  </div>
              ...
              <div>
                ...
                >
                  <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                    {selected.requirements.filter((r) => r.accepted).length} von {selected.requirements.length}{' '}
                    akzeptiert
                  </span>
                  {selected.status === 'planning' && !selected.workflowId && (
                    <button className="btn btn-primary btn-sm" onClick={handleGenerateWorkflow}>
                        ...
                        >
                          <button
                            style={{
                              ...
                              border: 'none',
                              color: req.accepted ? 'var(--bg-primary)' : 'transparent',
                              fontSize: 12,
                              display: 'flex',
                              alignItems: 'center',
                          ...
                          <span
                            className={`badge ${req.category === 'security' ? 'high' : 'info'}`}
                            style={{ fontSize: 9 }}
                          >
                            {req.category}
                          </span>
                ...
                <div>
                  {selected.files.length === 0 ? (
                    <div className="card" style={{ textAlign: 'center', padding: 24 }}>
                      ...
                      <button className="btn btn-primary btn-sm" onClick={handleGenerateFiles}>
                        Code generieren
                      </button>
                    </div>
                  ) : (
                    <div>
                      ...
                      >
                        <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                          {selected.files.length} Dateien &middot;{' '}
                          {(selected.files.reduce((s, f) => s + f.size, 0) / 1024).toFixed(1)} KB
                        </span>
                      </div>
                      {selected.files.map((file) => (
                        <div
                          ...
                          style={{
                            ...
                            borderLeftColor:
                              viewingFile === file.id
                                ? (langColors[file.language] ?? 'var(--accent-cyan)')
                                : 'var(--border-color)',
                          }}
                        ...
                        >
                          <div
                            style={{
                              ...
                              height: 8,
                              borderRadius: 2,
                              background: langColors[file.language] ?? 'var(--text-muted)',
                              flexShrink: 0,
                            }}
                          />
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>
                              {file.language} &middot; {(file.size / 1024).toFixed(1)} KB
                            </div>
                          </div>
                          <span
                            className={`badge ${file.status === 'approved' ? 'valid' : file.status === 'rejected' ? 'critical' : 'info'}`}
                            style={{ fontSize: 9 }}
                          >
                            {file.status}
                          </span>
                  ...
                  <div className="card" style={{ overflow: 'auto' }}>
                    ...
                    >
                      <div>
                        ...
                        <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>
                          {viewedFile.language} &middot; Agent: {viewedFile.generatedBy} &middot;{' '}
                          {new Date(viewedFile.generatedAt).toLocaleString('de-DE')}
                        </div>
                      </div>
                      <button className="btn btn-ghost btn-sm" onClick={() => setViewingFile(null)}>
                        {'\u2716'}
                      </button>
                    </div>
                    <pre
          ...
          >
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 12, marginTop: 4 }}>
                Geben Sie eine URL ein, um ein Projekt zu analysieren und zu erstellen
              </div>
              <div
                style={{
                  fontSize: 11,
