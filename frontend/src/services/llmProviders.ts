...
const openaiModels: LLMModel[] = [
  ...
  },
  {
    ...
    capabilities: ['text', 'code', 'function-calling', 'json-mode'],
    isLocal: false,
  },
  {
    id: 'o3',
...
const groqModels: LLMModel[] = [
  ...
  },
  {
    ...
    capabilities: ['text', 'code', 'json-mode'],
    isLocal: false,
  },
  {
...
/**
 * Merge any default providers that a persisted config is missing (e.g. a
 * newly added provider like xAI). Existing providers — and the user's keys /
 * enabled flags — are left untouched; only absent ones are appended. This lets
 * new providers appear for users who already have a saved llm_config.
 */
export function withProviderDefaults(cfg: LLMConfig): LLMConfig {
  if (!cfg?.providers?.length) return cfg;
  const have = new Set(cfg.providers.map((p) => p.id));
  const missing = defaultProviders.filter((d) => !have.has(d.id));
  return missing.length ? { ...cfg, providers: [...cfg.providers, ...missing] } : cfg;
}
