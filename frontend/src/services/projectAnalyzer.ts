...
function analyzeHtml(
  ...
  if (
    allSrc.includes('websocket') ||
    allHtml.includes('websocket') ||
    allHtml.includes('ws://') ||
    allHtml.includes('wss://')
  )
  ...
  const featureKeywords: Record<string, string> = {
    trading: 'Trading-Funktionalität',
    bot: 'Bot-Management',
    chart: 'Chart-Visualisierung',
    portfolio: 'Portfolio-Übersicht',
    dashboard: 'Dashboard',
    login: 'Authentifizierung',
    api: 'API-Integration',
    websocket: 'Echtzeit-Daten',
    table: 'Datentabellen',
    form: 'Formulare',
    modal: 'Modal-Dialoge',
    ...
    strategy: 'Strategie-Editor',
    risk: 'Risiko-Management',
    alert: 'Alert-System',
    history: 'Transaktions-Historie',
  };
...
export function generateRequirements(analysis: NonNullable<Project['analyzedStructure']>): ProjectRequirement[] {
  ...
  if (analysis.technologies.some((t) => t.includes('WebSocket'))) {
    reqs.push({
      ...
      description: 'Echtzeit-Datenverbindung via WebSocket implementieren',
      priority: 'high',
      accepted: true,
    });
  }
  ...
  reqs.push(
    ...
    },
    {
      ...
      description: 'Alle Eingaben serverseitig und clientseitig validieren',
      priority: 'high',
      accepted: true,
    },
    {
      ...
      category: 'testing',
      title: 'Unit Tests',
      description: 'Kernfunktionalitäten mit Unit Tests abdecken',
      priority: 'medium',
      accepted: true,
    },
    {
      id: `req_${++idx}`,
      category: 'infra',
      title: 'Build & Deploy Setup',
      description: 'Build-Pipeline und Deployment-Konfiguration',
      priority: 'low',
      accepted: true,
    },
  );
...
export function generateWorkflowFromProject(project: Project, agents: Agent[]): Workflow {
  ...
  for (const cat of categories) {
    ...
    steps.push({
      id: stepId,
      name: `${catLabels[cat] ?? cat} implementieren`,
      ...
      output: null,
      progress: 0,
      estimatedDuration: 15 + catReqs.length * 8,
      retries: 0,
    });
  }
  ...
  steps.push({
    id: integId,
    name: 'Integration & Zusammenführung',
    ...
    status: 'pending',
    dependsOn: Object.values(catSteps),
    output: null,
    progress: 0,
    estimatedDuration: 20,
    retries: 0,
...
`,
    ...
    files.push({
      id: `file_${++fileIdx}`,
      path: 'src/services/exchangeApi.ts',
      language: 'typescript',
...
`,
    ...
    files.push({
      id: `file_${++fileIdx}`,
      path: 'src/components/BotManager.tsx',
      language: 'typescript',
      content: `import { useState } from 'react';
