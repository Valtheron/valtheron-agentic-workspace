...
export default function App() {
  ...
  const [activeViewTab, setActiveViewTab] = useState<'sandbox' | 'docs_organizer' | 'database' | 'monitoring'>('sandbox');
...
router.post('/api/v1/agent/config', (req, res) => {
  ...
  // Batch refactor multiple messy documents sequentially for elegant visualization
  const handleBatchRefactor = async () => {
    if (batchSelectedPaths.length === 0) {
      showToast("Please select at least one document for batch standardization.", "error");
      return;
    }

    setIsLoading(true);
    setBatchProgress({
      total: batchSelectedPaths.length,
      current: 0,
      activeFile: '',
      isBatching: true
    });
    ...
    let successCount = 0;
    const importedDrafts: ContributionDraft[] = [];

    try {
      for (let i = 0; i < batchSelectedPaths.length; i++) {
        const docPath = batchSelectedPaths[i];
        const doc = MESSY_DOCS_STRUCTURE.find(d => d.originalPath === docPath);
        if (!doc) continue;

        setBatchProgress(prev => ({
          ...prev,
          current: i + 1,
          activeFile: doc.originalPath.split('/').pop()
        }));
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(`Failed to process ${doc.originalPath.split('/').pop()}`);
          }
          ...
          importedDrafts.unshift(serverDraft); // Add newest first
          successCount++;
        } catch (itemErr: any) {
          console.error(`Error during batch item processing for ${docPath}:`, itemErr);
          showToast(`Error processing ${doc.originalPath.split('/').pop()}: ${itemErr.message}`, "error");
        }
      }

      if (importedDrafts.length > 0) {
        setDrafts(prev => [...importedDrafts, ...prev]);
        // Set the most recently processed draft as active
        setCurrentDraft(importedDrafts[0].responseMarkdown);
        setActiveDraftId(importedDrafts[0].id);
      }

      showToast(`${t('toastBatchCompleted')} (${successCount}/${batchSelectedPaths.length})`, "success");
      fetchAuditLogs();
      setBatchSelectedPaths([]); // Clear selection after successful batch run
    } catch (error: any) {
      console.error(error);
      showToast(error.message || "Failed during batch standardization.", "error");
    } finally {
      setIsLoading(false);
      setBatchProgress(prev => ({ ...prev, isBatching: false }));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#070b13] font-sans text-gray-200 selection:bg-cyan-500 selection:text-black">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
            ...
            <div className="flex bg-gray-900 border border-gray-800 rounded-lg p-0.5 sm:p-1 text-[11px] sm:text-xs overflow-x-auto no-scrollbar max-w-full gap-0.5" id="header_view_tabs">
              ...
              <button 
                ...
                className={`px-2 py-1.5 sm:px-3 sm:py-1.5 rounded-md sm:rounded-lg font-medium transition duration-150 flex items-center gap-1 sm:gap-1.5 shrink-0 ${activeViewTab === 'monitoring' ? 'bg-cyan-500 text-black font-semibold' : 'text-gray-400 hover:text-white'}`}
              >
                <Activity className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                <span>
                  <span className="hidden sm:inline">
                    {lang === 'de' ? 'Monitoring' : lang === 'fr' ? 'Surveillance' : lang === 'pl' ? 'Monitorowanie' : 'Monitoring'}
                  </span>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 animate-fade-in" id="primary_main_section">
            ...
            <div className={`lg:col-span-7 space-y-6 ${isWidescreenDocs ? 'hidden' : ''}`}>
              ...
              <div className="bg-[#0b0f19] border border-gray-800 rounded-2xl p-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest font-mono block">
                  ...
                  <div className="flex gap-2 text-[10px]">
                    <button
                      onClick={() => {
                        const allMessyPaths = MESSY_DOCS_STRUCTURE.map(d => d.originalPath);
                        setBatchSelectedPaths(allMessyPaths);
                      }}
                      className="px-2 py-1 bg-gray-950 border border-gray-800 text-cyan-400 hover:border-cyan-700/50 rounded transition duration-150 cursor-pointer"
                    >
                      {t('selectAll')}
                    </button>
                    <button
                      ...
                      className="px-2 py-1 bg-gray-950 border border-gray-800 text-gray-400 hover:border-gray-700/50 rounded transition duration-150 cursor-pointer"
                    >
                      {t('deselectAll')}
                    </button>
                  </div>
                ...
                {/* Batch Actions & Progress panel */}
                {batchSelectedPaths.length > 0 && (
                  <div className="bg-[#0c1e28]/70 border border-cyan-800/40 rounded-xl p-4 flex flex-col gap-3 mb-4 animate-fadeIn" id="sandbox_batch_action_panel">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-mono font-bold text-cyan-300">
                          {t('selectedCount')}{batchSelectedPaths.length} / {MESSY_DOCS_STRUCTURE.length}
                        </span>
                      </div>
                      
                      {!batchProgress.isBatching ? (
                        <button
                          onClick={handleBatchRefactor}
                          disabled={isLoading}
                          className="px-4 py-1.5 bg-gradient-to-r from-emerald-500 to-cyan-500 hover:from-emerald-400 hover:to-cyan-400 text-black font-bold text-[11px] uppercase tracking-wider rounded-lg transition duration-150 flex items-center gap-1.5 shadow cursor-pointer"
                        >
                          <Sparkles className="w-3 h-3" />
                          <span>{t('btnBatchStandardize')}</span>
                        </button>
                      ) : (
                        <div className="text-[11px] font-mono text-cyan-400 flex items-center gap-1.5 bg-cyan-950/40 border border-cyan-800/30 px-2.5 py-1 rounded">
                          <Loader2 className="w-3.5 h-3.5 animate-spin" />
                          <span>{t('btnBatchStandardizing')}</span>
                        </div>
                      )}
                    </div>
                    ...
                    {batchProgress.isBatching && (
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-[10px] font-mono text-gray-300">
                          <span>Standardizing: <strong className="text-cyan-400 font-bold">{batchProgress.activeFile}</strong></span>
                ...
                <div className="space-y-3">
                  {MESSY_DOCS_STRUCTURE.map((doc) => {
                    ...
                    const isBatchChecked = batchSelectedPaths.includes(doc.originalPath);
                      ...
                      <div
                        ...
                        className={`cursor-pointer text-left p-3.5 rounded-xl border transition-all duration-200 flex flex-col md:flex-row justify-between md:items-center gap-3 ${
                          isSelected 
                            ...
                            : isBatchChecked
                              ? 'bg-cyan-950/5 border-cyan-950/50'
                      ...
                      >
                        <div className="flex gap-3 items-start min-w-0 flex-1">
                          ...
                          <div 
                            onClick={(e) => {
                              e.stopPropagation();
                              const alreadySelected = batchSelectedPaths.includes(doc.originalPath);
                              if (alreadySelected) {
                                setBatchSelectedPaths(batchSelectedPaths.filter(p => p !== doc.originalPath));
                              } else {
                                setBatchSelectedPaths([...batchSelectedPaths, doc.originalPath]);
                              }
                            }}
                            className="mt-1.5 shrink-0 flex items-center justify-center cursor-pointer select-none"
                            id={`sandbox_checkbox_toggle_${doc.originalPath.replace(/[^a-zA-Z0-9]/g, '_')}`}
                          >
                            <div className={`w-4 h-4 rounded border transition-all duration-150 flex items-center justify-center ${
                              isBatchChecked
                                ...
                                : 'bg-gray-950 border-gray-700 hover:border-cyan-500/55 text-transparent'
                            }`}>
        ...
        {/* Dynamic Tab 4 View: Comprehensive Monitoring-Dashboard */}
        {activeViewTab === 'monitoring' && (
          <div className="animate-fade-in" id="monitoring_view_container">
            <MonitoringDashboard 
              lang={lang}
